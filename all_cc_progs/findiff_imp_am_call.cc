...
double option_price_call_american_finite_diff_implicit(const double& S,
    ...
    int M; if ((no_S_steps%2)==1) { M=no_S_steps+1; } else { M=no_S_steps; }; // need no_S_steps to be even:
    ...
    A.element(M,M)=1.0;
    ColumnVector B(M+1);
    for (unsigned m=0;m<=M;++m){ B.element(m) = max(0.0,S_values[m]-K); };
    ColumnVector F=A.i()*B;
    for(int t=N-1;t>0;--t) {
	B = F;
	F = A.i()*B;
	for (unsigned m=1;m<M;++m) { 	// now check for exercise
